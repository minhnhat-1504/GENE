...
from sklearn.preprocessing import StandardScaler  # Import thêm để chuẩn hóa tại đây

# Import module nội bộ
import config, data_loader, fitness_wrapper, pa5_runner, fs_mapping

# --- HÀM TÍNH ĐỘ ỔN ĐỊNH (JACCARD) ---
def calculate_stability_post_hoc(mask_list):
    """Tính chỉ số Jaccard trung bình giữa các lần chạy liên tiếp"""
...
def execute_single_run(run_id, algo_type, X_train, y_train, X_test, y_test):
    ...
    if algo_type == config.ALGO_NAME:
        ...
        curr_z, curr_fit, hist = pa5_runner.run_ga(X_train, y_train, fitness_wrapper.fitness_feature_selection)
    elif algo_type == "PSO":
        curr_z, curr_fit, hist = pa5_runner.run_pso(X_train, y_train, fitness_wrapper.fitness_feature_selection)

    # 2. Đánh giá kết quả cuối cùng trên tập Test
    ...
    if len(sel_idx) > 0:
        # Quan trọng: n_jobs=1 để tránh xung đột CPU khi chạy song song nhiều tiến trình
        ...
        cm = confusion_matrix(y_test, y_pred) # Lưu ma trận nhầm lẫn
    
    return {
        'Run': run_id + 1,
        'Accuracy': acc,
        'Fitness': curr_fit,
        'Selected_Genes': len(sel_idx),
        'Sensitivity': sens,
        'Specificity': spec,
        'Time': time.time() - start_t,
        'Mask': m,       # Để tính Stability
        'History': hist, # Để vẽ biểu đồ hội tụ
        'CM': cm         # Để vẽ Heatmap
    }

# --- MAIN PROCESS ---
def main():
    # Cấu hình cho i5-11400H (12 luồng logic -> dùng 10 luồng là an toàn)
    MAX_WORKERS = 10 
    
    print(f"PARALLEL SESSION STARTED: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Workers: {MAX_WORKERS} | Runs per Algo: {config.R_RUNS}")

    datasets = [
        ("Golub", config.DATASETS["Golub_Leukemia"]),
        ("CuMiDa", config.DATASETS["CuMiDa_GSE9476"])
    ]
    algos = [config.ALGO_NAME, "GA", "PSO"]
    ...
    for data_name, data_cfg in datasets:
        ...
        # 1. LOAD DATA & PREPROCESSING (QUAN TRỌNG: FIX DATA LEAKAGE)
        # Lưu ý: Đảm bảo data_loader.py đã được sửa để trả về dữ liệu THÔ (chưa scale)
        
        if data_cfg.get('is_split'):
            # --- Trường hợp Golub (Đã chia sẵn file Train/Test) ---
            ...
            # --- Trường hợp CuMiDa (Chưa chia) ---
            X_all, y_all, _ = data_loader.load_dataset(data_cfg['path'], data_cfg['label_col'])
            
            # B1: Chia Train/Test TRƯỚC
            X_train_raw, X_test_raw, y_train, y_test = train_test_split(
                X_all, y_all, test_size=config.TEST_SIZE, stratify=y_all, random_state=42
            )
            
            # B2: Chuẩn hóa SAU khi chia (Tránh lộ thông tin tập Test sang tập Train)
            scaler = StandardScaler()
            X_train = scaler.fit_transform(X_train_raw)
            X_test = scaler.transform(X_test_raw)
        
        # Lưu tên lớp (Class Names) để vẽ biểu đồ sau này
        ...
        for algo in algos:
            ...
            with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
                futures = {
                    executor.submit(execute_single_run, r, algo, X_train, y_train, X_test, y_test): r 
            ...
            # --- XỬ LÝ DỮ LIỆU SAU KHI CHẠY XONG ---
            results_list.sort(key=lambda x: x['Run'])
            
            # A. Tìm Run tốt nhất để lưu Confusion Matrix
            # Ưu tiên Accuracy cao nhất, sau đó là Fitness thấp nhất
